var modalSaveFile = [
    ...
    ["Autonomous Disaster Response and Reconnaissance Bot | Lidar & vSLAM",
     "mobile_robot",
     "mr.png",
     "Applied techniques of mobile robotic kinematics and sensing, motion planning and SLAM, to conduct autonomous reconnaissance in an initially unknown real-world environment designed to simulate a disaster scenario. Specifically, when introduced into a closed but initially unknown environment, the system accomplishes the following: 1. Generate a complete map of the environment; 2. Locate any victims present in the environment. For the purposes of this exercise, the environment is represented using an occupancy grid map, and AprilTags are used as stand-ins for simulated victims. Our objective is to deploy a ground robot in an unknown environment filled with AprilTags at randoom locations in random poses. The bot must generate a comprehensive map of the area while also identifying and cataloguing all the AprilTags present, including their unique ID numbers and absolute poses relative to the generated map. We utilized a 360° LiDAR, wheel encoders and a camera to avoid obstacles while generate occupancy grid maps, integrating custom ROS nodes with existing packages like Cartographer to achieve robust exploration and accurate victim localization in the map. ",
     "See the demo video!",
